...
pub struct JwtRule {
    ...
    /// List of header locations from which JWT is expected. For example, below is the location spec if JWT is expected to be found in `x-jwt-assertion` header, and have \"Bearer \" prefix: ```yaml fromHeaders: - name: x-jwt-assertion prefix: \"Bearer \" ```
    #[serde(rename = "fromHeaders", skip_serializing_if = "Option::is_none")]
    pub from_headers: Option<Vec<super::JwtHeader>>,
    /// List of query parameters from which JWT is expected. For example, if JWT is provided via query parameter `my_token` (e.g /path?my_token=<JWT>), the config is: ```yaml fromParams: - \"my_token\" ```
