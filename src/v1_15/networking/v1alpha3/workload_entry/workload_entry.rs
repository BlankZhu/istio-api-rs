...
/// WorkloadEntrySpec : WorkloadEntrySpec enables specifying the properties of a single non-Kubernetes workload such a VM or a bare metal services that can be referred to by service entries.



#[derive(Clone, Debug, PartialEq, Default, Serialize, Deserialize, JsonSchema, CustomResource)]
#[kube(group = "networking.istio.io", version = "v1alpha3", kind = "WorkloadEntry", namespaced)]
pub struct WorkloadEntrySpec {
    ...
    /// The service account associated with the workload if a sidecar is present in the workload. The service account must be present in the same namespace as the configuration ( WorkloadEntrySpec or a ServiceEntry)
...
impl WorkloadEntrySpec {
    /// WorkloadEntrySpec enables specifying the properties of a single non-Kubernetes workload such a VM or a bare metal services that can be referred to by service entries.
    pub fn new() -> WorkloadEntrySpec {
        WorkloadEntrySpec {
            address: None,
            labels: None,
            locality: None,
